...
def writeSVG(sourceFileName, dirDestination):
	...
	dest.write("function Icon({width = 32, height = 32, ...props}) {\n")
	dest.write("  return (\n")
	lines = source.readlines()
	for line in lines:
		if (line.startswith("<svg")):
			line = line.replace("<svg","<svg {...props}")
		line = line.replace("width=\"32\"","width={width}")
		line = line.replace("height=\"32\"","height={height}")
...
def writeImage(src, address, dirDestination, size):
	...
	dest.write("import React from 'react';\n\n")
	dest.write("function Image({width = 32, height = 32, ...props}) {\n")
	dest.write("\treturn (\n")
